    ...
    <div class="px-3 py-2 header d-flex align-items-center justify-content-center">
        <div class="container d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
            ...
            <nav class="hidden">
                @if (Route::has('login'))
                    @auth
                        <a href="{{ url('/home') }}" class="header-link mr-4">Home</a>
                    @else
                        <a href="{{ route('login') }}" class="header-link mr-4">Entrar</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="mx-4 header-link">Cadastrar-se</a>
                        @endif
                    @endauth
                @endif
            </nav>
